...
--- ONLY USE FOR TESTING on environements WITHOUT an existing 'Survey' table ---
/*******************************************************************************
-- leadr."Survey" --------------------------------------------------------------
...
----- ONLY USE IF REQUIRED to create entries into the 'SurveySeries' table -----
/*******************************************************************************
-- SurveySeries ----------------------------------------------------------------
...
SELECT
    '0x1c45f',    -- alpha: 'Organization UID'
    '0x674891',   -- alpha: 'Leadr Coach'
    T."ID",
    T."Configuration",
    current_timestamp
FROM leadr."Template" AS T;
...
CREATE TEMP TABLE Results (table_name varchar(50), status bool, count smallint);

INSERT INTO Results
SELECT 'Template', EXISTS(SELECT FROM information_schema."tables" WHERE "table_name" = 'Template');

INSERT INTO Results
SELECT 'Question', EXISTS(SELECT FROM information_schema."tables" WHERE "table_name" = 'Question');

INSERT INTO Results
SELECT 'SurveySeries', EXISTS(SELECT FROM information_schema."tables" WHERE "table_name" = 'SurveySeries');

INSERT INTO Results
SELECT 'SurveySeriesSurvey', EXISTS(SELECT FROM information_schema."tables" WHERE "table_name" = 'SurveySeriesSurvey');

INSERT INTO Results
SELECT 'QuestionSeries', EXISTS(SELECT FROM information_schema."tables" WHERE "table_name" = 'QuestionSeries');

INSERT INTO Results
SELECT 'QuestionSeriesQuestion', EXISTS(SELECT FROM information_schema."tables" WHERE "table_name" = 'QuestionSeriesQuestion');

WITH cte_Template AS (
  SELECT COUNT(*) count
  FROM leadr."Template"
)
UPDATE Results
   SET count = cte_Template.count
  FROM cte_Template
...
UPDATE Results
   SET count = cte_Question.count
  FROM cte_Question
 WHERE table_name = 'Question';

WITH cte_SurveySeries AS (
  SELECT COUNT(*) count
  FROM leadr."SurveySeries"
)
UPDATE Results
   SET count = cte_SurveySeries.count
  FROM cte_SurveySeries
 WHERE table_name = 'SurveySeries';

WITH cte_SurveySeriesSurvey AS (
  SELECT COUNT(*) count
  FROM leadr."SurveySeriesSurvey"
)
UPDATE Results
   SET count = cte_SurveySeriesSurvey.count
  FROM cte_SurveySeriesSurvey
 WHERE table_name = 'SurveySeriesSurvey';

WITH cte_QuestionSeries AS (
  SELECT COUNT(*) count
  FROM leadr."QuestionSeries"
)
UPDATE Results
   SET count = cte_QuestionSeries.count
  FROM cte_QuestionSeries
 WHERE table_name = 'QuestionSeries';

WITH cte_QuestionSeriesQuestion AS (
  SELECT COUNT(*) count
  FROM leadr."QuestionSeriesQuestion"
)
UPDATE Results
   SET count = cte_QuestionSeriesQuestion.count
  FROM cte_QuestionSeriesQuestion
 WHERE table_name = 'QuestionSeriesQuestion';
...
WHERE
    ...
    AND t.relkind = 'r'
    AND t.relname IN ( 'Template'
                      ,'Question'
                      ,'SurveySeries'
                      ,'SurveySeriesSurvey'
                      ,'QuestionSeries'
                      ,'QuestionSeriesQuestion' )
ORDER BY
    t.relname,
    a.attnum,
    i.relname;
