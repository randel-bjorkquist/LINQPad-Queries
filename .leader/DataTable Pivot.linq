...
static void Main()
{
     ...
     string[] uniqueQuarters = dt.AsEnumerable()
                                 .Select(x => x.Field<string>("Quarter"))
                                 .Distinct()
                                 .ToArray();

     var groups = dt.AsEnumerable()
                    .GroupBy(x => x.Field<int>("PillarId"))
                    .Select(x => x.GroupBy(y => y.Field<string>("Quarter"))
                                  .Select(z => new { id      = x.Key
                                                    ,quarter = z.Key
                                                    ,feature = z.Select((a, i) => new { feature = a.Field<string>("Feature")
                                                                                       ,index = i })
                                                                .ToList()})
                                  .ToList())
                    .ToList();
     ...
     {
         int maxNewRows = group.Select(x => x.feature.Count())
                               .Max();
         ...
         for (int index = 0; index < maxNewRows; index++)
         {
             ...
             foreach (var row in group)
             {
                 ...
                 newRow[row.quarter] = row.feature.Skip(index) == null || !row.feature.Skip(index).Any() 
                                     ? "" 
                                     : row.feature.Skip(index).First().feature;
             }
         }
     }
