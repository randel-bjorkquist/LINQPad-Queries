...
void Main()
{
  ...
  var combined = mcsEventType.InspectionSaved | mcsEventType.FamilyCertSaved;
  combined.HasFlag(mcsEventType.InspectionSaved).Dump("combined.HasFlag(mcsEventType.InspectionSaved)");
  combined.HasFlag(mcsEventType.FamilyCertSaved).Dump("combined.HasFlag(mcsEventType.FamilyCertSaved)");
}
  ...
  where Tid   : notnull, IEquatable<Tid>, IComparable<Tid>
{
  ...
  protected static mcsSmartEnum<TSelf, Tid> None = (TSelf)Activator.CreateInstance(typeof(TSelf), default(Tid), "None", nameof(None));
  
  /// <summary>
  /// Combines two TypedEnum values using bitwise OR.
  /// </summary>
  public static mcsSmartEnum<TSelf, Tid> operator | (mcsSmartEnum<TSelf, Tid> left, mcsSmartEnum<TSelf, Tid> right)
  {
